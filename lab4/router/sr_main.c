...
time_t icmp_id_timeout = 60;
time_t tcp_established_idle_timeout = 7440;
time_t tcp_transitory_idle_timeout = 300;
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "nhs:v:p:u:t:r:l:T:I:E:R:")) != EOF)
    {
        ...
        {
	        case 'n':
                sr_nat_init(&(sr.nat_icmp));
                sr_nat_init(&(sr.nat_tcp));
                sr.inbound_syn_list = NULL;
                break;
            case 'h':
                usage(argv[0]);
                exit(0);
            ...
            case 'T':
                ...
                break;
            case 'I':
                icmp_id_timeout = atoi((char *) optarg);
                break;
            case 'E':
